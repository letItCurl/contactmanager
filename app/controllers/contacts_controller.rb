class ContactsController < ProtectedController
  ...
  def index
    ...
    @contacts = current_user.contacts.by_group(params[:group_id]).search(params[:term]).order(:name).page(params[:page])
  end
  ...
  def autocomplete
    @contacts = current_user.contacts.search(params[:term]).order(:name).page(params[:page])
